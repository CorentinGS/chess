...
func TestGameUnsafePushMoveTextRejectsSAN(t *testing.T) {
	_, err := chess.NewGame().UnsafeMoveText("e4", chess.SAN(), nil)
	if !errors.Is(err, chess.ErrUnsafeMoveTextUnsupported) {
		t.Fatalf("UnsafeMoveText(SAN) error = %v, want ErrUnsafeMoveTextUnsupported", err)
	}
}

func TestGameUnsafePushMoveTextAcceptsIllegalCoordinateMove(t *testing.T) {
	game := chess.NewGame()
	if _, err := game.UnsafeMoveText("e2e5", chess.UCI(), nil); err != nil {
		t.Fatalf("UnsafeMoveText() error = %v", err)
	}
...
func TestGameUnsafePushMoveTextClassifiesSpecialMoves(t *testing.T) {
	t.Run("capture from UCI", func(t *testing.T) {
		game := chess.NewGame()
		mustUnsafeMoveText(t, game, "e2e4", chess.UCI())
		mustUnsafeMoveText(t, game, "d7d5", chess.UCI())
		node := mustUnsafeMoveText(t, game, "e4d5", chess.UCI())
	...
	t.Run("castling from UCI", func(t *testing.T) {
		...
		node := mustUnsafeMoveText(t, game, "e1g1", chess.UCI())
	...
	t.Run("en passant from long algebraic", func(t *testing.T) {
		...
		node := mustUnsafeMoveText(t, game, "e5xd6", chess.LongAlgebraic())
	...
	t.Run("null move from UCI", func(t *testing.T) {
		game := chess.NewGame()
		node := mustUnsafeMoveText(t, game, "0000", chess.UCI())
...
func mustUnsafeMoveText(t *testing.T, game *chess.Game, moveText string, codec chess.MoveTextCodec) *chess.MoveNode {
	t.Helper()
	node, err := game.UnsafeMoveText(moveText, codec, nil)
	if err != nil {
		t.Fatalf("UnsafeMoveText(%q, %s) error = %v", moveText, codec, err)
	}
	return node
}
