...
// TestCursorUndoRoundTrip walks the game tree with the cursor's slim
// make/unmake pair and asserts that unmakeMoveCursor restores the exact
// pre-move position — FEN and hash — at every node. This is the lockstep
// guarantee for board.unapply's reversal of promotion, en passant, and
// castling effects.
func TestCursorUndoRoundTrip(t *testing.T) {
	fens := []string{
		"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
		"r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",
		"8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1",
	...
	for _, fen := range fens {
		...
		walk = func(p *Position, depth int) {
			...
			preFEN := p.String()
			preHash := p.ZobristHash()
			for _, m := range p.ValidMoves() {
				u := p.makeMoveCursor(m)
				walk(p, depth-1)
				p.unmakeMoveCursor(m, u)
				if got := p.String(); got != preFEN {
					t.Fatalf("fen %s move %v: after unmake got %s, want %s", fen, m, got, preFEN)
				}
				if got := p.ZobristHash(); got != preHash {
					t.Fatalf("fen %s move %v: hash drift after unmake", fen, m)
				}
			}
		}
		walk(pos, 3)
